package com.quasimondo.bitmapdata
{
	...
	public class CameraBitmap extends EventDispatcher
	{
		...
		public static const E_CAMERA_ALLOW:String = "Event_CameraAllow";
		public static const E_CAMERA_DENY:String = "Event_CameraDeny";
		public static const E_CAMERA_RENDER:String = "Event_CameraRender";
		...
		public function CameraBitmap( width:int, height:int, refreshRate:int = 15, cameraWidth:int = -1, cameraHeight:int = -1 )
		{
			...
			__cam.addEventListener(StatusEvent.STATUS, onCamStatusHandler, false, 0, true);
		...
		private function onCamStatusHandler(e:StatusEvent):void
		{
			if (__cam.muted == true)
			{
				dispatchEvent(new Event(E_CAMERA_ALLOW));
			}
			else
			{
				dispatchEvent(new Event(E_CAMERA_DENY));
			}
		}
		...
		private function paint( event:TimerEvent = null ):void
		{
			...
			bitmapData.unlock();
			dispatchEvent( new Event( E_CAMERA_RENDER ) );
		}
