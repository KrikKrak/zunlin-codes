...
package com.zzl.flex.photoGallery.view
{
	...
	public class Reflector extends UIComponent
	{
		...
		public function set target(value: UIComponent): void {
			...
			if (_target != null) {
				...
				// redrawn as well.
				_target.addEventListener(FlexEvent.UPDATE_COMPLETE, handleTargetUpdate, true, 0, true);

				// Register to get notified whenever the target moves or resizes.
				_target.addEventListener(MoveEvent.MOVE, handleTargetMove, false, 0, true);
				_target.addEventListener(ResizeEvent.RESIZE, handleTargetResize, false, 0, true);
		...
		public function set falloff(value: Number): void {
			...
			//reset the height of this reflector
			if(_target != null)
			{
		...
		override protected function updateDisplayList(unscaledWidth: Number, unscaledHeight: Number): void
		{
			...
			if (_target != null) {
				...
				//var fallHeight:Number = _target.height * _falloff;
				//unscaledHeight = fallHeight;
