...
package cn.riahome.transformer
{
	...
	public class DisplayObjectTransformer
	{
		 ...
		 *  @returns 返回裁剪后的 BitmapData 对象。
		 */
		public static function cutOutSuper( target:DisplayObject, template:DisplayObject, isAnti:Boolean = false ):BitmapData
		{
		 ...
		 * 
		 * @param target 需要进行九切片缩放的 Bitmap 对象。
		 * @param newWidth 新宽度，如不想改变宽度，请将原值传递进来。
		 * @param newHeight 新高度，如不想改变高度，请将原值传递进来。
		 * @param scale9Grid 缩放网格。
		 * 
		 */		
		public static function scale9(target:Bitmap, newWidth:Number, newHeight:Number, scale9Grid:Rectangle = null):void
		{
			
			if (scale9Grid)
				target.scale9Grid = scale9Grid;
			
			var bmpData:BitmapData = new BitmapData(newWidth, newHeight, true, 0x00000000);
			
			var rows:Array = [0, target.scale9Grid.top, target.scale9Grid.bottom, target.height];
			var cols:Array = [0, target.scale9Grid.left, target.scale9Grid.right, target.width];
			var dRows:Array = [0, target.scale9Grid.top, newHeight - (target.height - target.scale9Grid.bottom), newHeight];
			var dCols:Array = [0, target.scale9Grid.left, newWidth - (target.width - target.scale9Grid.right), newWidth];
			...
			for (var cx:int = 0; cx < 3; cx++)
			{
				for (var cy:int = 0; cy < 3; cy++)
				{
					...
					bmpData.draw(target, m, null, null, drawRect, target.smoothing);
				}
			}
			
			target.bitmapData.dispose();
			target.bitmapData = bmpData;
		}
